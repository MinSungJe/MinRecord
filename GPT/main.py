...
def get_summary_by_gpt(text: str):
    ...
    response = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "당신은 글을 요약하고 핵심 주제를 한 문장으로 정리해주는 어시스턴트입니다."},
            {"role": "user", "content": f"{text} 이 글을 전체적으로 요약해서 정리해줘. 그리고 그 정리된 내용이 모여 이야기하고자 하는 핵심적인 주제를 한 문장으로 마지막에 적어줘."}
        ],
    )
